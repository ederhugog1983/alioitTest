<template>
  <section>
    <v-row>
      <v-col v-if="$store.state.pokesCRUD.showPokeApi" cols=12 style="border-radius:60px;max-width: 90%;padding: 2%;margin: 3%; background:linear-gradient(90deg, rgba(86, 86, 86, 0.7) 0%, rgba(86, 86, 86, 0.7) 40%, rgba(86, 86, 86, 0.7) 80%)">
        <h1 style="font-size:24px">Container: {{$store.state.pokesCRUD.containerSelected.name}}</h1>
        <h3 style="font-size:20px">Pokes request to API</h3>
      </v-col>
    </v-row>
    <v-row>
    <v-col class="mb-4" cols=6 v-if="$store.state.pokesCRUD.showPokeApi">
    ...
    <v-col class="mb-4" cols=6 v-if="$store.state.pokesCRUD.showPokeApi">
        <v-btn style="border: solid 1px white" class="" @click="savePokeList()">
          <img  v-bind="attrs" v-on="on"
            :src="LoadedPokeIcon"
            height="20px"
            width="20px"
    ...
    <div v-if="$store.state.pokesCRUD.showPokeApi">
      <v-data-table
        v-model="selected"
        :headers="headers"
        :items="$store.state.pokesCRUD.pokesApi"
        item-key="name"        
        class="elevation-1"
      >
      </v-data-table>
...
import BackIcon from '@/assets/images/layout/components/table/back.svg'
 export default {
    ...
    computed: {
      pokes () {          
        return this.$store.state.pokesCRUD.pokesApi
      }
    },
    methods: {
      savePokeList() {
        const data = {
          pokesContainerId: this.$store.state.pokesCRUD.containerSelected._id,
          pokes: this.pokes
        }
        this.$store.dispatch('pokesCRUD/savePokesToContainer', data)
        this.$store.dispatch('pokesCRUD/showContainers', true)
        this.$store.dispatch('pokesCRUD/showPokeApi', false)
